...
const Repayments: React.FC = () => {
  const [list, setList] = useState<Repayment[]>([]);
  ...
  const [repayerType, setRepayerType] = useState('Contact');
  ...
  const updateList = async () => {
    const res = await axios.get(`/bi/loan/list/repayment?page=${currentPage}&limit=${pageSize}`);
    setList(res.data?.repayments);
  ...
  const handleOk = async () => {
    await axios.post(`/bi/loan/create/repayment`, {
  ...
  return <>
    <Modal width={350} title={`New Repayment`} open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
      ...
      <Text strong>Repayer: </Text>
      <Select
        options={[{ value: 'Contact', label: 'Contact' }, { value: 'User', label: 'User' }]}
        value={repayerType}
        onChange={(value) => setRepayerType(value)}
        className="form-field"
      />
      <TextArea
        rows={2}
